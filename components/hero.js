...
import FridgeOpen from './fridgeOpen';
...
export default function Hero() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            const downArrow = arrowRef.current;
        ...
        setTimeout(() => {
            if (arrowRef.current) {
                arrowRef.current.classList.add(styles.moveArrow);
            }
        }, 2500);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (scrollPosition > 2500) {
                ...
                setShowHeroIntro(true);
                if (logoRef.current && titleRef.current && subLogoRef.current) {
